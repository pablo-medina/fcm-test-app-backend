...
class FCMClient {
    ...
    constructor(serviceAccountKey, { messageTTL } = {}) {
    ...
    async _doNotificationRequest({ token, title, body, image }) {
        const messageTTL = this._messageTTL;
        let apnTTL;
        if (messageTTL) {
            apnTTL = messageTTL > 0 ? Math.floor(Date.now() / 1000) + messageTTL : 0;
        }
        ...
        if (messageTTL) {
            ...
            androidData["ttl"] = `${messageTTL}s`;
            apnsData["headers"] = {
                'apns-expiration': `${apnTTL}`, // Fecha de expiración UNIX epoch en segundos para APNs (iOS)
            }
        }
    ...
    async _renewAccessToken() {
        ...
        const auth = new GoogleAuth({
            credentials: this._serviceAccountKey,
